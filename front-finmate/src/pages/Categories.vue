<template>
  <div class="dashboard-container">
    ...
    <v-card>
      ...
      <v-alert v-if="store.error" class="ma-4" closable density="compact" rounded="lg" type="error" variant="tonal"
        @click:close="store.error = ''">
        {{ store.error }}
      </v-alert>

      <v-data-table class="pa-2" :headers="headers" hide-default-footer :items="filteredCategories" :items-per-page="-1"
        ...
        <template #item.isSystem="{ item }">
          <v-chip v-if="item.isSystem" color="blue" size="small" variant="tonal">
            Sistema
          </v-chip>
        </template>
        <template #item.actions="{ item }">
          <v-btn :disabled="item.isSystem" icon size="small" :title="item.isSystem ? 'No se puede editar' : 'Editar'"
            variant="text" @click="openEdit(item)">
            <v-icon>mdi-pencil</v-icon>
          </v-btn>
          <v-btn color="error" :disabled="item.isSystem" icon size="small"
            :title="item.isSystem ? 'No se puede eliminar' : 'Eliminar'" variant="text" @click="confirmDelete(item)">
        ...
        <v-card-text class="pa-4">
          ...
          <v-form @submit.prevent="handleSave">
            <div class="fm-field-group">
              ...
              <v-text-field v-model="form.name" class="fm-input" density="comfortable" hide-details="auto"
                placeholder="Ej: Salario" required rounded="lg" variant="outlined" />
            </div>
            ...
            <div class="fm-field-row">
              <div class="fm-field-group">
                ...
                <v-text-field v-model="form.icon" class="fm-input" density="comfortable" hide-details="auto"
                  placeholder="mdi-food" rounded="lg" variant="outlined" />
              </div>
              <div class="fm-field-group">
                <label class="fm-label">Color (opcional)</label>
                <v-text-field v-model="form.color" class="fm-input" density="comfortable" hide-details="auto"
                  placeholder="#FF5733" rounded="lg" variant="outlined" />
              </div>
            </div>
    ...
    <v-dialog v-model="deleteDialogOpen" max-width="400">
      <v-card>
        ...
        <v-divider />
        <v-card-text class="pa-4">
          <p>¿Estás seguro de eliminar la categoría <strong>{{ deletingItem?.name }}</strong>?</p>
...
async function handleSave() {
  ...
  try {
    ...
    await (editingId.value ? store.updateCategory(editingId.value, payload as UpdateCategoryBody) : store.createCategory(payload))
